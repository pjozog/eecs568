...
public class VictoriaListener {
    ...
    JFrame jf    = new JFrame("Victoria Park");
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            VictoriaListener dt = new VictoriaListener(new ConfigFile(args[0]));


            // Read from file, add to slam, draw stuff
            String pathToData = "./src/team/sim/victoriaPark.txt";
            ...
            while (scan.hasNext()) {
                ...
                if (type.equals("ODOMETRY")) {
                    ...
                    dt.newLandMeasurement(nodeIndexOne, nodeIndexTwo, deltaX, deltaY);
    ...
    public void newMeasurement(int indexOne,
        ...
        cov.set(0, 0, 1/10000.0);
        cov.set(1, 1, 1/250000.0);
        cov.set(2, 2, 1/250000.0);
    ...
    public void newLandMeasurement(int indexOne,
                                   int indexTwo,
                                   double deltaX,
                                   double deltaY) {
        ...
        if (indexTwo > numNodes) {
            p2dnTwo = new Point2DNode();
            slam.addNode(p2dnTwo);
            numNodes++;
        } else {
            p2dnTwo = (Point2DNode)slam.getNodes().get(indexTwo);
            // System.out.println("Getting old node two");
        }

        Point2D deltaMotion = new Point2D(deltaX, deltaY);

        Matrix cov = Matrix.identity(2, 2);
        cov = cov.times(0.4001);

        Pose2DToPoint2DEdge poseToPose = new Pose2DToPoint2DEdge(p2dnOne, p2dnTwo, deltaMotion, cov);

        slam.addEdge(poseToPose);

        slam.update();

        if (numConstraints % 50 == 0) {
            drawSetup();
            drawScene();
        }
    ...
    public double getChiGuy() {
        return slam.getNormalizedChi2();
    }


    public VictoriaListener(Config config) {
    ...
    public void drawScene() {
        ...
        // Draw the landmarks
        {
            ...
            for (double[] l : landmarks) {
                vb.addBack(new VisChain(LinAlg.translate(l[0],l[1],0.0),
