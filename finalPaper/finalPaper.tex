...
% environment, and localization within their current belief of the world. 
...
% Previous work that applies \ac{SLAM} to quadrotor helicopters was often accomplished using 



%  To enable realtime
% operations, techniques must be used to expedite the solving of least-squares problems. 


% We used two basic techniques, first, not solving the portions of the problem that have not
% changed since the last computation, and second, preserving and exploiting the sparsity in
% matrices inherent in our problem.


 %  In Section~\ref{sec:incrementalsmoothingandmapping}, we
...
% in~Section~\ref{sec:backendModels}.

% In~Section~\ref{sec:experiments}, we experimentally apply our solver to a specific robot, the
