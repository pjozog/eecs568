...
representation of any least-squares problem.  This representation was successfully applied
to the \ac{SLAM} problem in~\cite{dellaert2005square}.  These factors can relate two robot
nodes, or a landmark node and a robot node.  Factors are the constraints that the back-end
...
cameras for recording images in the forward-looking and downward-looking directions~\cite{ardronedevguide}.
...
% Pat's section
For the simulated data, we used the simulator developed by the April lab for the EECS568
homeworks. When evaluating different \ac{SLAM} back-ends, we seeded the random number
generator to ensure the exact same data was being generated. The data from the simulator
is from a 2D world, so we set the z, roll, and pitch observations to be zero with high
certainty. Because of the flexibility in our \ac{SLAM} back-end, it would be easy to
create nodes and edges for a 2D world. However, we chose to use the 3D nodes and edges to
see the system grow at the same rate of a full 6-\ac{DOF} application.

The three different methods of \ac{SLAM} that we evaluated had enormous speed differences.
Table \ref{tab:timing} shows the total simulation time for two loops of the
...
We ran our back-end on the Manhattan dataset from~\cite{olson2006fast} to assess the
computational performance and accuracy of our implementation.  Our final normalized
$\chi^2$ error value is 1.0399274, which is comparable to that of iSAM's.  A plot of the
$\chi^2$ error over time is shown in Figure~\ref{fig:manhattan}.  On a cheap
workstation (less than \$1000 US), we finished the computations in about 253 seconds.
...
\begin{figure*}[!t]
  \begin{center}
    ...
    \subfigure[Normalized $\chi^2$ error] {
      \includegraphics[width=2.5in]{images/chi2}
      \label{fig:manhattanChi2}}
  \end{center}
  \caption{\subref{fig:manhattanResult} shows the final result of our SLAM back-end on the
    Manhattan dataset. The normalized $\chi^2$ error vs. number of constraints shown in
    \subref{fig:manhattanChi2} is quite similar to that of iSAM's.}
  \label{fig:manhattan}
\end{figure*}
...
To interactively demonstrate our \ac{SLAM} back-end to the layman, we used a
consumer-grade webcam tracking a pre-assigned AprilTag to simulate odometry.  Five other
unique tags acting as positional landmarks were added to the scene, and tracked from a
camera being waved by a student.  A screen-capture of the program is shown in
...
unfold in a 3D \texttt{Vis} scene proved to be a creative visualization of the problem,
...
association with acceptable quality and $\chi^2$ error.  Note that our back-end is agnostic
of the data association problem, and can be used with front-ends that solve for unknown
data association SLAM problems.
